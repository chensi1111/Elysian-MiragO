...
export const useShoppingCartStore = defineStore("shoppingCart", () => {
  ...
  function removeItem(
    ...
    ElMessageBox.confirm(`確定要刪除${productname}嗎?`, "Warning", {
  ...
  function switchColor(
    ...
    if (itemIndex !== -1) {
      ...
      const productList =
        currentItem.type === "man"
          ? productInfoStore.manProductList
          : productInfoStore.womanProductList;
      const newProduct = productList.find(
        (product) => product.name === productName && product.color === newColor
      );
  ...
  function switchSize(
    ...
    if (itemIndex !== -1) {
      ...
      const newProduct = productList.find(
        (product) =>
          product.name === productName && product.color === productColor
      );
