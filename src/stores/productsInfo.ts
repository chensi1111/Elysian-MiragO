...
export const useProductsInfoStore = defineStore('productsInfo', () => {
    ...
    onMounted(async () => {
        ...
        const querySnapshotForAccessory = await getDocs(collection(db, "accessory"));
        firestoreDataForMan.value = querySnapshotForMan.docs.map(doc => doc.data());
        firestoreDataForWoman.value = querySnapshotForWoman.docs.map(doc => doc.data());
        firestoreDataForAccessory.value = querySnapshotForAccessory.docs.map(doc => doc.data());
    });
    const updateManProducts = () => {
        originalManProductList.value = firestoreDataForMan.value.map((data: any) => {
            return reactive({
                name: data.name,
                price: data.price,
                img: data.URL,
    ...
    const updateWomanProducts = () => {
        originalWomanProductList.value = firestoreDataForWoman.value.map((data: any) => {
            return reactive({
                name: data.name,
                price: data.price,
                img: data.URL,
    ...
    const updateAccessoryProducts = () => {
        accessoryProductList.value = firestoreDataForAccessory.value.map((data: any) => {
            return reactive({
                name: data.name,
                price: data.price,
                img: data.URL,
    ...
    const manProductList = computed(() => {
        return [
            ...originalManProductList.value,
            ...accessoryProductList.value
        ];
    })
    const womanProductList = computed(() => {
        return [
            ...originalWomanProductList.value,
            ...accessoryProductList.value
        ];
    })
